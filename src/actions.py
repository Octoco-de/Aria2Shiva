...
def torrent_button(torrent, movie_id, index):
    ...
    return InlineKeyboardButton(
        text=f"{torrent['quality']} {size}", callback_data=f"dowM:{movie_id}:{index}"
    )
...
async def search_movie(update, context, chat_id, query):
    ...
    logger.info(f"search movie {query}")
    try:
        movies = await search(query)
        logger.info(f"yts response: {movies}")
...
async def user_selected_movie(update, context, chat_id, movie_id):
    try:
        ...
        movie_data = await get_movie_details(
            movie_id
        )  # Store the movie_id in user_data
        temp_array = []
        buttons = []
...
async def display_summary(update, context, chat_id, movie_id):
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id, f"`{movie_data['summary']}`", parse_mode="Markdown"
        )
    except Exception:
        await context.bot.send_message(chat_id, "Well this is embarrassing....")
...
async def download_torrent(
    update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id, url
):
    try:
        ...
        existing_sessions_output = subprocess.check_output(
            ["tmux", "list-sessions"]
        ).decode()
        if tmux_session_name in existing_sessions_output:
            await context.bot.send_message(
                chat_id, "This download has already started!"
            )
            return
        ...
        directory = (
            ...
            else os.path.expanduser("~/Torrents")
        )
        ...
        await context.bot.send_message(
            chat_id,
            "Oops! There was an error starting the download. Please try again later.",
        )
    except Exception as e:
        print(f"Unknown error: {e}")
        await context.bot.send_message(
            chat_id, "Oops! Something went wrong. Please try again later."
        )
